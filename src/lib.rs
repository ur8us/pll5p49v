...
/// Write configuration registers - embedded-hal 1.x.x
pub fn write_config_blocking<I2C, E>(
    i2c: &mut I2C,
    clock_fq_hz: u32,
    vco_fq_hz: u32,
    output1_fq_hz: u32,
    ...
    I2C: I2c<Error = E>,
{
    ...
    i2c.write(I2C_ADDRESS, &prog_array)
}

/// Write configuration registers - async
pub async fn write_config_async<I2C, E>(
    i2c: &mut I2C,
    clock_fq_hz: u32,
    vco_fq_hz: u32,
    output1_fq_hz: u32,
...
/// Calibrate VCO - async
pub async fn calibrate_vco_async<I2C, E>(i2c: &mut I2C) -> Result<(), E>
where
    I2C: I2c_async<Error = E>,
{
...
/// Calibrate VCO - embedded-hal 1.x.x
pub fn calibrate_vco_blocking<I2C, E>(i2c: &mut I2C) -> Result<(), E>
where
    I2C: I2c<Error = E>,
{
    // Read 7th bit of the register 0x1C
    ...
    let mut resp_buff = [0u8; 1];
    i2c.write_read(I2C_ADDRESS, &[0x1C], &mut resp_buff)?;

    i2c.write(I2C_ADDRESS, &[0x1c, resp_buff[0] & 0x7F])?;

    // Timer::after_millis(1).await;

    i2c.write(I2C_ADDRESS, &[0x1c, resp_buff[0] | 0x80])?;
    ...
    let mut resp_buff = [0u8; 1];
    i2c.write_read(I2C_ADDRESS, &[0x99], &mut resp_buff)?;
